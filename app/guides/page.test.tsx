...
describe("GuidesPage", () => {
  it("lists the complete attraction guide catalog", () => {
    ...
    expect(screen.getByRole("link", { name: /梵蒂冈博物馆/ })).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /米兰大教堂与露台/ })).toBeInTheDocument();
  });
  ...
  it("filters guides by city", async () => {
    ...
    expect(screen.queryByRole("link", { name: /米兰大教堂与露台/ })).not.toBeInTheDocument();
  });
});
