...
describe('guide catalog', () => {
  ...
  it.each([
    ...
    expect(guideForTripItem(first!)?.slug).toBe(
  ...
  it('renders Vatican and Saint Peter as native guide chapters', () => {
    ...
    for (const slug of reusedSlugs) {
      ...
      expect(guide).not.toHaveProperty('embeddedGuide');
      expect(guide?.spatial.type).toBe('floorplan');
      expect(guide?.spatial.stops.length).toBeGreaterThanOrEqual(8);
