  ...
  return (
    <main className="subpage">
      ...
      <header className="page-intro page-intro--timeline">
        ...
        <h1>逐日行程</h1>
        <p>完全按现有行程顺序呈现。步行与交通耗时合并在下一个景点的到达说明中，备选项目放在每天末尾。</p>
      ...
      <div className="filter-bar" aria-label="按城市筛选">
        {["全部", ...tripCities].map((city) => (
          <button data-active={cityFilter === city} key={city} onClick={() => setCityFilter(city)} type="button">
            {city}
          </button>
        ))}
      </div>
      ...
      <div className="itinerary-content">
        {filteredDays.map((day) => <DayTimeline day={day} key={day.date} />)}
      </div>
    </main>
  );
}
