...
export async function githubPreview(output = 'dist/client', basePath = '/europe-cultural-guide-2026', unavailablePaths = new Set(), observe = () => {}) {
  const root = resolve(output);
  const server = createServer(async (request, response) => {
    try {
      const pathname = decodeURIComponent(new URL(request.url, 'http://localhost').pathname);
      observe(pathname, request.headers);
