...
try {
  ...
  const image = await page.evaluate(async () => {
    ...
    const { buildSighsExterior } = await import('/app/lib/sighs-exterior-model.ts');
    const { perspectiveBoxFitDistance } = await import('/app/lib/exterior-geometry.ts');
    const model = buildSighsExterior(T, mergeGeometries);
    ...
    const renderer = new T.WebGLRenderer({ antialias: true }); renderer.setSize(1440, 1200);
    renderer.toneMapping = T.ACESFilmicToneMapping; renderer.toneMappingExposure = 1.12;
    const camera = new T.PerspectiveCamera(35, 1440 / 1200, 0.1, 250);
    const bounds = new T.Box3().setFromObject(model), center = bounds.getCenter(new T.Vector3()), direction = new T.Vector3(7, 4, 16).normalize();
    ...
    renderer.dispose(); renderer.forceContextLoss(); return data;
  });
  const bytes = await sharp(Buffer.from(image.split(',')[1], 'base64')).webp({ quality: 92 }).toBuffer();
  await writeFile('public/models/sighs-exterior.webp', bytes);
  console.log(JSON.stringify({ bytes: bytes.length, dimensions: [1440, 1200] }));
